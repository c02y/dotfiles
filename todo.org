...
** non-GUI
   ...
   emacs-git w3m jq moreutils poppler xdot calc scc stylua watchexec
   proxychains wget transmission-cli yt-dlp shellcheck-static shfmt luarocks cppcheck upx hunspell neomutt npm nodejs 
   clang gdb cmake bear meson ninja sccache ccls powertop auto-cpufreq
...
** GUI
   ...
   krita krita-plugin-gmic copyq nomacs kimageformats zathura zathura-pdf-poppler
   noto-fonts noto-fonts-emoji noto-fonts-cjk font-manager flameshot peek rofi thunderbird 
   firefox google-chrome audacious easytag

   pandoc (depends on a lot of haskell, install pandoc-bin from archlinuxcn) plank
