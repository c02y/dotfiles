...
local thumbnailer_options = {
	...
	autogenerate_max_duration = 36000, -- 1 hour
...
function window_controls(topbar)
	...
	lo.geometry = { x = titlebox_left, y = wc_geo.y - 3, an = 1, w = titlebox_w, h = wc_geo.h }
...
function osc_init()
	...
	ne.eventresponder["mouse_move"] = --keyframe seeking when mouse is dragged
		function(element)
			...
			if (element.state.lastseek == nil) or not (element.state.lastseek == seekto) then
				mp.commandv("seek", seekto, "absolute-percent", user_opts.seekbarkeyframes and "keyframes" or "exact")
	...
	ne.content = function()
		...
		if dmx_cache and (dmx_cache > state.dmx_cache * 1.1 or dmx_cache < state.dmx_cache * 0.9) then
